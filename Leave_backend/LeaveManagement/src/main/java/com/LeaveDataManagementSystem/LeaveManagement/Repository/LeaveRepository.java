...
public interface LeaveRepository extends MongoRepository<Leave, String> {
    ...
    // ═══════════════════════════════════════════════════════════════════
    // NEW: userId-based lookups (preferred going forward — stable even if
    // the employee's email changes or gets reused by someone else)
    // ═══════════════════════════════════════════════════════════════════
    ...
    @Query("{ 'userId': ?0, $or: [ " +
            ...
            "{ $and: [ { 'startDate': { $lte: ?1 } }, { 'endDate': { $gte: ?2 } } ] } " +
            "] }")
    List<Leave> findOverlappingLeavesByUserId(String userId, LocalDate startDate, LocalDate endDate);

    @Query("{ 'userId': ?0, 'startDate': { $gte: ?1 }, 'endDate': { $lte: ?2 } }")
    List<Leave> findByUserIdAndDateRange(String userId, LocalDate startDate, LocalDate endDate);

    List<Leave> findByUserIdAndStatusOrderByCreatedAtDesc(String userId, LeaveStatus status);

    @Query(value = "{ 'userId': ?0, 'startDate': { $gte: ?1, $lte: ?2 } }", count = true)
    long countByUserIdAndYear(String userId, LocalDate yearStart, LocalDate yearEnd);

    @Query("{ 'userId': ?0, 'startDate': { $gte: ?1, $lte: ?2 } }")
    List<Leave> findByUserIdAndYear(String userId, LocalDate yearStart, LocalDate yearEnd);

    @Query("{ 'userId': ?0, 'status': 'APPROVED', 'isCancelled': false, 'startDate': { $gte: ?1, $lte: ?2 } }")
    List<Leave> findApprovedLeavesByUserIdAndDateRange(String userId, LocalDate startDate, LocalDate endDate);

    @Query("{ 'userId': ?0, 'isCancelled': false, 'startDate': { $gt: ?1 }, " +
            "'status': { $nin: ['REJECTED_BY_ACTING_OFFICER', 'REJECTED_BY_SUPERVISING_OFFICER', 'REJECTED_BY_APPROVAL_OFFICER'] } }")
    List<Leave> findCancellableLeavesByUserId(String userId, LocalDate currentDate);

    List<Leave> findByUserIdAndIsCancelledTrueOrderByCreatedAtDesc(String userId);

    @Query("{ 'userId': ?0, 'startDate': { $lte: ?2 }, 'endDate': { $gte: ?1 }, " +
            "'status': { $nin: ['REJECTED_BY_ACTING_OFFICER', 'REJECTED_BY_SUPERVISING_OFFICER', 'REJECTED_BY_APPROVAL_OFFICER'] } }")
    List<Leave> findOverlappingLeavesExcludingRejectedByUserId(String userId, LocalDate startDate, LocalDate endDate);

    // Count leaves by department, joined via userId now instead of email
    @Aggregation(pipeline = {
            "{ $lookup: { from: 'Users', localField: 'userId', foreignField: '_id', as: 'employee' } }",
            "{ $unwind: '$employee' }",
            "{ $group: { _id: '$employee.department', count: { $sum: 1 } } }"
    })
    List<Map<String, Object>> countLeavesByDepartmentViaUserId();

    // ═══════════════════════════════════════════════════════════════════
    // EXISTING email-based methods — kept as-is for backward compatibility
    // ═══════════════════════════════════════════════════════════════════
    List<Leave> findByEmployeeEmailOrderByCreatedAtDesc(String employeeEmail);
    List<Leave> findByActingOfficerEmailAndStatusOrderByCreatedAtAsc(String actingOfficerEmail, LeaveStatus status);
    List<Leave> findByApprovalOfficerEmailAndStatusOrderByCreatedAtAsc(String approvalOfficerEmail, LeaveStatus status);
